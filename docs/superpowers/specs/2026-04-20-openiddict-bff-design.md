...
`Brigade.WebHost` (Blazor SSR) acts as three things in a single process:
...
|---|---|---|---|---|
| `brigade-bff` | Confidential | AuthorizationCode + PKCE | Implicit | Blazor BFF; redirect URI `https://localhost:7000/signin-oidc`; post-logout `https://localhost:7000/signout-callback-oidc` |
| `brigade-api-test` | Confidential | ClientCredentials | — | Test client for hello-world API |

**Scopes:**

| Name | Resources |
|---|---|
| `brigade-api` | `brigade-webhost` |
...
The `"authDb"` connection name is already wired up to `Brigade.WebHost`.
