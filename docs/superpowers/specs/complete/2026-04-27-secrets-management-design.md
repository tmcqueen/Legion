...
Legion stores sensitive values — provider API keys (`ProviderOptions.ApiToken`), MCP server header values (`McpServerHeaders.Value`), and MCP server command-line strings (`McpServerOptions.CommandLine`) — as plain strings in the database. There is no mechanism to manage these values centrally, rotate them, or protect them at rest.
...
A new `SecretOptions` record is added to `Legion.Admin.Data.Models`. **No changes are made to existing models** (`ProviderOptions`, `McpServerOptions`, `McpServerHeaders`). Those fields remain `string?` and will hold either a raw value or a `secret://` URI.
...
|---|---|---|
| `Legion.Admin.Data.PostgreSQL` | `PostgreSqlSecretsStore` | pgcrypto raw SQL |
| `Legion.Admin.Data.Sqlite` | `SqliteSecretsStore` | EF Core, plaintext |
...
A `DelegatingHandler` is added to every `HttpClient` that Legion uses to call AI providers. Before each outbound request is sent, the handler scans all request headers for `secret://` references and resolves them via `ISecretsManager`.
...
- Added to named `HttpClient` instances used by `AgentFactory` in `Legion.Agents`.
- `Legion.Agents.csproj` gains a project reference to `Legion.Admin.Data` to access `ISecretsManager`.
...
| File | Purpose |
|---|---|
| `src/libs/Legion.Admin.Data.Models/SecretOptions.cs` | Data model |
| `src/libs/Legion.Admin.Data/Stores/SecretsStore.cs` | `ISecretsStore` interface + base |
| `src/libs/Legion.Admin.Data.PostgreSQL/Stores/PostgreSqlSecretsStore.cs` | pgcrypto store |
| `src/libs/Legion.Admin.Data.Sqlite/Stores/SqliteSecretsStore.cs` | Plaintext store |
| `src/libs/Legion.Admin.Data/Services/SecretsManager.cs` | `ISecretsManager` implementation |
...
| File | Change |
|---|---|
| `src/libs/Legion.Admin.Data/AppDbContext.cs` | Add `DbSet<SecretOptions>` |
...
| `src/Legion.Agents/Legion.Agents.csproj` | Add reference to `Legion.Admin.Data` |
| `src/Legion.Agents/Providers/AgentFactory.cs` | Accept `ISecretsManager`, resolve secrets before building agents |
| Provider / MCP Server edit forms in `Legion.Admin.UI` | Add secret mode toggle |
