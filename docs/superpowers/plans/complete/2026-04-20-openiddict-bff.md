...
**Goal:** Complete the OpenIddict authorization server in Legion.WebHost, enabling cookie-based BFF authentication for Blazor users (Authorization Code + PKCE) and bearer token access for external services (Client Credentials), protected by a hello-world API controller.

**Architecture:** Legion.WebHost is simultaneously the OpenIddict server, the Blazor BFF, and the resource server. Users authenticate via Authorization Code flow against the same host (self-referencing OIDC); the session cookie holds the tokens server-side. External services POST clientId+secret to `/connect/token` for a bearer token. Both token types are validated by OpenIddict's local validation middleware.
...
**Modified:**
- `src/Legion.WebHost/appsettings.Development.json` — fix connection string key names
- `src/Legion.WebHost/appsettings.Migrations.json` — fix connection string key names
- `src/Legion.WebHost/Legion.WebHost.csproj` — add `Aspire.Npgsql.EntityFrameworkCore.PostgreSQL`
- `src/Legion.WebHost/Data/AuthDbContext.cs` — add `OnModelCreating` / `UseOpenIddict()`
- `src/Legion.WebHost/Program.cs` — full rewrite: Identity, DbContext, OpenIddict, Cookie+OIDC, middleware pipeline
- `src/Legion.WebHost/Components/Routes.razor` — `AuthorizeRouteView` + `CascadingAuthenticationState`

**Created:**
- `src/Legion.WebHost/Controllers/HelloWorldController.cs` — protected `GET /api/hello`
- `src/Legion.WebHost/Controllers/AuthorizationController.cs` — `/connect/authorize`, `/connect/token`, `/connect/logout`
- `src/Legion.WebHost/Services/OpenIddictSeedService.cs` — startup seeder for apps + scope
- `src/Legion.WebHost/Services/CookieRevalidatingAuthenticationStateProvider.cs` — Blazor circuit auth revalidation
- `src/Legion.WebHost/Components/Pages/Account/Login.razor` — SSR login form
- `src/Legion.WebHost/Components/Pages/Account/Logout.razor` — SSR sign-out page
- `tests/Legion.WebHost.Tests/Legion.WebHost.Tests.csproj` — xUnit test project
- `tests/Legion.WebHost.Tests/Controllers/HelloWorldControllerTests.cs` — unit tests

---

## Task 1: Fix connection string key names

Aspire injects connection strings as `ConnectionStrings:legionDb` and `ConnectionStrings:authDb` (matching the names in `AppHost.cs`). Both `appsettings.Development.json` and `appsettings.Migrations.json` currently use `"legion"` / `"auth"` which causes `GetConnectionString("legionDb")` to return null.

**Files:**
- Modify: `src/Legion.WebHost/appsettings.Development.json`
- Modify: `src/Legion.WebHost/appsettings.Migrations.json`
...
```bash
git add src/Legion.WebHost/appsettings.Development.json \
        src/Legion.WebHost/appsettings.Migrations.json
git commit -m "fix: align connection string keys with Aspire names (legionDb, authDb)"
```
...
**Files:**
- Modify: `src/Legion.WebHost/Legion.WebHost.csproj`

- [ ] **Step 1: Add the package**

```bash
cd src/Legion.WebHost
dotnet add package Aspire.Npgsql.EntityFrameworkCore.PostgreSQL
```

- [ ] **Step 2: Verify the entry in the csproj**

Open `src/Legion.WebHost/Legion.WebHost.csproj` and confirm:
```xml
...
```bash
git add src/Legion.WebHost/Legion.WebHost.csproj
...
**Files:**
- Modify: `src/Legion.WebHost/Data/AuthDbContext.cs`
...
```bash
git add src/Legion.WebHost/Data/AuthDbContext.cs \
        src/Legion.WebHost/Data/Migrations/
...
**Files:**
- Create: `tests/Legion.WebHost.Tests/Legion.WebHost.Tests.csproj`
- Create: `tests/Legion.WebHost.Tests/Controllers/HelloWorldControllerTests.cs`

- [ ] **Step 1: Create the xUnit project**

```bash
mkdir -p tests/Legion.WebHost.Tests
cd tests/Legion.WebHost.Tests
dotnet new xunit --name Legion.WebHost.Tests --output .
dotnet add reference ../../src/Legion.WebHost/Legion.WebHost.csproj
...
Expected: compile error — `Legion.WebHost.Controllers.HelloWorldController` not found.

- [ ] **Step 4: Commit**

```bash
git add tests/Legion.WebHost.Tests/
...
**Files:**
- Create: `src/Legion.WebHost/Controllers/HelloWorldController.cs`
...
namespace Legion.WebHost.Controllers;
...
```bash
git add src/Legion.WebHost/Controllers/HelloWorldController.cs
...
**Files:**
- Create: `src/Legion.WebHost/Controllers/AuthorizationController.cs`
...
```bash
git add src/Legion.WebHost/Controllers/AuthorizationController.cs
...
Registers the two OIDC applications and the `legion-api` scope on startup, idempotently.

**Files:**
- Create: `src/Legion.WebHost/Services/OpenIddictSeedService.cs`
...
namespace Legion.WebHost.Services;
    ...
    ILogger<OpenIddictSeedService> logger) : IHostedService
{
    ...
    {
        if (await scopeManager.FindByNameAsync("legion-api", ct) is not null) return;
    ...
    {
        const string clientId = "legion-bff";
    ...
    private async Task SeedApiTestApplicationAsync(
        ...
        CancellationToken ct)
    {
        const string clientId = "legion-api-test";
        if (await appManager.FindByClientIdAsync(clientId, ct) is not null) return;
...
```bash
git add src/Legion.WebHost/Services/OpenIddictSeedService.cs
...
**Files:**
- Create: `src/Legion.WebHost/Services/CookieRevalidatingAuthenticationStateProvider.cs`
...
```bash
git add src/Legion.WebHost/Services/CookieRevalidatingAuthenticationStateProvider.cs
...
**Files:**
- Create: `src/Legion.WebHost/Components/Pages/Account/Login.razor`
...
<PageTitle>Sign in — Legion</PageTitle>
...
```bash
git add src/Legion.WebHost/Components/Pages/Account/Login.razor
...
**Files:**
- Create: `src/Legion.WebHost/Components/Pages/Account/Logout.razor`
...
```bash
git add src/Legion.WebHost/Components/Pages/Account/Logout.razor
...
**Files:**
- Modify: `src/Legion.WebHost/Program.cs`
...
Expected: clean build — no CS errors. If the `legion-bff` OIDC client secret is missing, it will throw at startup (not at build time).

- [ ] **Step 3: Commit**

```bash
git add src/Legion.WebHost/Program.cs
...
**Files:**
- Modify: `src/Legion.WebHost/Components/Routes.razor`
...
```bash
git add src/Legion.WebHost/Components/Routes.razor
...
**Prerequisites:** PostgreSQL running via Aspire (`dotnet run` in `src/Legion.AppHost`). User secrets set in Task 13.

- [ ] **Step 1: Start Aspire**

```bash
cd src/Legion.AppHost
dotnet run
```
...
API_SECRET=$(dotnet user-secrets get "OpenIddict:ApiTestClientSecret" \
    --project src/Legion.WebHost 2>/dev/null || echo "<paste-secret-here>")

# Request a token
curl -sk -X POST https://localhost:7000/connect/token \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "grant_type=client_credentials&client_id=legion-api-test&client_secret=${API_SECRET}&scope=legion-api"
```
