...
| File | Responsibility |
|---|---|
| `src/libs/Legion.Admin.Data.Models/SecretOptions.cs` | Entity record |
| `src/libs/Legion.Admin.Data/Configurations/SecretOptionsConfiguration.cs` | EF fluent config |
| `src/libs/Legion.Admin.Data/Stores/SecretsStore.cs` | `ISecretsStore` interface |
| `src/libs/Legion.Admin.Data/Services/SecretsManager.cs` | `ISecretsManager` interface + implementation |
| `src/libs/Legion.Admin.Data.Sqlite/Stores/SqliteSecretsStore.cs` | EF plaintext store |
| `src/libs/Legion.Admin.Data.PostgreSQL/Stores/PostgreSqlSecretsStore.cs` | pgcrypto raw-SQL store |
| `src/Legion.Agents/Http/SecretResolvingHandler.cs` | Outbound HTTP handler |
...
| `tests/Legion.Secrets.Tests/Legion.Secrets.Tests.csproj` | Test project |
| `tests/Legion.Secrets.Tests/SecretsManagerTests.cs` | SecretsManager unit tests |
| `tests/Legion.Secrets.Tests/SqliteSecretsStoreTests.cs` | SqliteSecretsStore integration tests |
| `tests/Legion.Secrets.Tests/SecretResolvingHandlerTests.cs` | Handler unit tests |
| `tests/Legion.Secrets.Tests/SecretsControllerTests.cs` | Controller unit tests |
| EF migrations (PostgreSQL + SQLite) | `Secrets` table |

### Modified files
| File | Change |
|---|---|
| `src/libs/Legion.Admin.Data/AppDbContext.cs` | Add `DbSet<SecretOptions> Secrets` |
| `src/libs/Legion.Admin.Data/Extensions/AgentDbContextExtensions.cs` | Register `ISecretsManager` |
| `src/libs/Legion.Admin.Data.PostgreSQL/Extensions/PostgreSqlExtensions.cs` | Register `PostgreSqlSecretsStore` |
| `src/libs/Legion.Admin.Data.Sqlite/Extensions/SqliteExtensions.cs` | Register `SqliteSecretsStore` |
| `src/Legion.Agents/Legion.Agents.csproj` | Add `<ProjectReference>` to `Legion.Admin.Data` |
| `src/Legion.Agents/Providers/AgentFactory.cs` | Accept `ISecretsManager`, resolve `ApiToken` before building agents |
| `src/libs/Legion.Admin.UI/Components/Sidebar.razor` | Add "Secrets" nav item |
| `src/libs/Legion.Admin.UI/Pages/Providers/Providers.Update.razor` | Secret mode toggle for `ApiToken` |
| `src/libs/Legion.Admin.UI/Pages/Mcps/Mcps.Update.razor` | Secret mode toggle for `CommandLine` + header `Value` |
...
**Files:**
- Create: `src/libs/Legion.Admin.Data.Models/SecretOptions.cs`

- [ ] **Step 1: Create the model**

```csharp
// src/libs/Legion.Admin.Data.Models/SecretOptions.cs
...
Create `src/libs/Legion.Admin.Data/Configurations/SecretOptionsConfiguration.cs`:
...
```bash
cd /home/timm/Legion/.worktrees/features/secrets-management
...
git add src/libs/Legion.Admin.Data.Models/SecretOptions.cs \
        ...
        src/libs/Legion.Admin.Data/AppDbContext.cs
...
**Files:**
- Create: `src/libs/Legion.Admin.Data/Stores/SecretsStore.cs`
- Create: `src/libs/Legion.Admin.Data/Services/SecretsManager.cs`
...
**Files:**
- Create: `tests/Legion.Secrets.Tests/Legion.Secrets.Tests.csproj`
- Create: `tests/Legion.Secrets.Tests/SecretsManagerTests.cs`

- [ ] **Step 1: Create test project**

```bash
dotnet new xunit -n Legion.Secrets.Tests -o tests/Legion.Secrets.Tests --framework net10.0
dotnet sln Legion.slnx add tests/Legion.Secrets.Tests/Legion.Secrets.Tests.csproj
```

- [ ] **Step 2: Add project reference to Legion.Admin.Data**

Edit `tests/Legion.Secrets.Tests/Legion.Secrets.Tests.csproj`:
...
```bash
dotnet test tests/Legion.Secrets.Tests/ --filter "SecretsManagerTests" -v minimal
```
...
```bash
dotnet restore tests/Legion.Secrets.Tests/
dotnet test tests/Legion.Secrets.Tests/ --filter "SecretsManagerTests" -v minimal
```
...
```bash
git add tests/Legion.Secrets.Tests/ Directory.Packages.props Legion.slnx
...
**Files:**
- Create: `src/libs/Legion.Admin.Data.Sqlite/Stores/SqliteSecretsStore.cs`

- [ ] **Step 1: Write failing store integration tests**

Create `tests/Legion.Secrets.Tests/SqliteSecretsStoreTests.cs`:
...
Add project reference to `Legion.Admin.Data.Sqlite` in test csproj:

```xml
<ProjectReference Include="..\..\src\libs\Legion.Admin.Data.Sqlite\Legion.Admin.Data.Sqlite.csproj" />
```
...
```bash
dotnet test tests/Legion.Secrets.Tests/ --filter "SqliteSecretsStoreTests" -v minimal
```
...
git add src/libs/Legion.Admin.Data.Sqlite/Stores/SqliteSecretsStore.cs \
        ...
        tests/Legion.Secrets.Tests/Legion.Secrets.Tests.csproj
...
**Files:**
- Create: `src/libs/Legion.Admin.Data.PostgreSQL/Stores/PostgreSqlSecretsStore.cs`
...
```bash
git add src/libs/Legion.Admin.Data.PostgreSQL/Stores/PostgreSqlSecretsStore.cs
...
**Files:**
- Create: migrations in `src/libs/Legion.Admin.Data.Sqlite/Migrations/`
- Create: migrations in `src/libs/Legion.Admin.Data.PostgreSQL/Migrations/AgentDb/`
...
dotnet ef migrations add AddSecrets \
  --project src/libs/Legion.Admin.Data.Sqlite \
  --startup-project src/WebDev \
  --context AppDbContext \
  --output-dir Migrations
```

Expected: Migration file created at `src/libs/Legion.Admin.Data.Sqlite/Migrations/<timestamp>_AddSecrets.cs`
...
```bash
dotnet ef migrations add AddSecrets \
  --project src/libs/Legion.Admin.Data.PostgreSQL \
  --startup-project src/WebDev \
  --context AppDbContext \
  --output-dir Migrations/AgentDb
```

Expected: Migration file created at `src/libs/Legion.Admin.Data.PostgreSQL/Migrations/AgentDb/<timestamp>_AddSecrets.cs`
...
dotnet ef database update \
  --project ../libs/Legion.Admin.Data.Sqlite \
  --context AppDbContext \
...
git add src/libs/Legion.Admin.Data.Sqlite/Migrations/ \
        src/libs/Legion.Admin.Data.PostgreSQL/Migrations/AgentDb/
...
**Files:**
- Modify: `src/libs/Legion.Admin.Data/Extensions/AgentDbContextExtensions.cs`
- Modify: `src/libs/Legion.Admin.Data.Sqlite/Extensions/SqliteExtensions.cs`
...
```bash
git add src/libs/Legion.Admin.Data/Extensions/AgentDbContextExtensions.cs \
...
**Files:**
- Create: `src/Legion.Agents/Http/SecretResolvingHandler.cs`
- Modify: `src/Legion.Agents/Legion.Agents.csproj`
- Modify: `src/Legion.Agents/Providers/AgentFactory.cs`
- Create: `tests/Legion.Secrets.Tests/SecretResolvingHandlerTests.cs`
...
Add project reference in `Legion.Secrets.Tests.csproj`:

```xml
<ProjectReference Include="..\..\src\Legion.Agents\Legion.Agents.csproj" />
```

- [ ] **Step 2: Run tests to verify they fail**

```bash
dotnet test tests/Legion.Secrets.Tests/ --filter "SecretResolvingHandlerTests" -v minimal
```

Expected: FAIL — `SecretResolvingHandler` does not exist

- [ ] **Step 3: Add project reference to Legion.Agents.csproj**
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <ItemGroup>
    <ProjectReference Include="..\libs\Legion.Admin.Data\Legion.Admin.Data.csproj" />
  </ItemGroup>
</Project>
```
...
> **Note:** `AgentOptions` is defined in `Legion.Agents`. Check whether it has an `ApiToken` property — if not, verify the actual property name by reading `src/Legion.Agents/Providers/AgentOptions.cs` and adjust accordingly.
...
```bash
git add src/Legion.Agents/Http/SecretResolvingHandler.cs \
        src/Legion.Agents/Legion.Agents.csproj \
...
- Create: `tests/Legion.Secrets.Tests/SecretsControllerTests.cs`
...
```bash
dotnet test tests/Legion.Secrets.Tests/ --filter "SecretsControllerTests" -v minimal
```
...
**Files:**
- Modify: `src/libs/Legion.Admin.UI/Components/Sidebar.razor`
- Modify: `src/libs/Legion.Admin.UI/Pages/Providers/Providers.Update.razor`
- Modify: `src/libs/Legion.Admin.UI/Pages/Mcps/Mcps.Update.razor`

- [ ] **Step 1: Add Secrets to sidebar**

Modify `src/libs/Legion.Admin.UI/Components/Sidebar.razor` — add after the Workflows item:
...
In `src/libs/Legion.Admin.UI/Pages/Providers/Providers.Update.razor`, replace the `ApiToken` RadzenRow with this mode-toggle version:
...
In `src/libs/Legion.Admin.UI/Pages/Mcps/Mcps.Update.razor`, replace the `CommandLine` RadzenRow:
...
Modify `src/libs/Legion.Admin.UI/Components/HeaderEditDialog.razor` — in the Value input row, add a mode toggle identical to the patterns above, keyed off `headerValueSecretMode`. When in secret mode show a plain `RadzenTextBox` for the `secret://` URI; when in raw mode show the existing input.

Detect mode on open: `headerValueSecretMode = InitialValue?.StartsWith("secret://") == true;`

- [ ] **Step 5: Build Legion.Admin.UI**

```bash
dotnet build src/libs/Legion.Admin.UI/Legion.Admin.UI.csproj
```
...
```bash
dotnet test Legion.slnx -v minimal
```
...
- **AgentOptions.ApiToken:** The field name needs verification — check `src/Legion.Agents/Providers/AgentOptions.cs` before writing `AgentFactory.cs`. The property may be named differently.
...
- **Dialog components location:** The `CreateSecretDialog`, `SecretValueDialog`, and `DescriptionDialog` are in `src/WebDev/Components/`, not in `Legion.Admin.UI`, because they're specific to this page and reference `ISecretsStore` which lives in the data layer (not the shared UI library).
